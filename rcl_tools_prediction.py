...
DATA_SET = "rcl_dataset_prediction/dan.txt"
...
def train_model(
    ...
    if block_for_ready:
        ...
        health_status = training_ready_check(
            session_key, InferencePriorityType.index, InferenceDetailType.predict_next
        )
        while health_check_count < health_check_threshold:
            health_status = training_ready_check(
                session_key,
                InferencePriorityType.index,
                InferenceDetailType.predict_next,
            )
...
def sample_prediction_set(path: Path, sample_size: int = 100, vector_size: int = 5):
    # predict line > vector length
    # predictions are of size vector_size so samples must be longer than that
    keep = []
    lines = path.read_text(encoding="utf8").strip().split("\n")
    for l in lines:
        l = l.split("\t")[0].strip()
        if len(l.split(" ")) > vector_size:
            keep.append(l)
    ...
    for i in range(iterations):
        ...
        data = [
            (
                ...
                p := inference(
                    session_key,
                    predict_line_resize(s, vector_size),
                    InferencePriorityType.index,
                    InferenceDetailType.predict_line,
                ),
                p == s,
                sentence_gleu([s.split()], p.split()),
            )
            for s in sample
        ]
        # data = [(source, prediction, accuracy, gleu)]

        accuracy = round(sum(map(lambda x: int(x[2]), data)) / len(sample), 2)
        gleu_avg = round(sum(map(lambda x: x[3], data)) / len(sample), 2)
...
def predict_example():
    ...
    print("Example inference input:" + sample)
    ...
    evaluate_predict_model(session_key, dataset)
